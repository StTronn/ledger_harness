// skill.ts — generates SKILL.md, the playbook the §8 agent is handed (SPEC §8:
// "the playbook is provided to the agent as a Skill describing the entry types and
// when each applies — same source of truth the rule engine uses; generate the
// skill text from the schema file so they can't drift"). The entry-type catalogue
// and param shapes are read from config/playbook.json; the decision rules and the
// inclusive-GST formula are pinned here so the Skill and the stub brain encode the
// SAME judgment.
import { readFileSync } from "node:fs";
import { resolve } from "node:path";

// playbook.json lives at the repo root under config/. Resolve it relative to this
// module (agent/src/skill.ts -> ../../config/playbook.json) so it is found whether
// the service is started from the repo root or from agent/.
const PLAYBOOK_PATH =
  process.env.LEDGER_FLOW_PLAYBOOK ??
  resolve(import.meta.dirname, "..", "..", "config", "playbook.json");
...
interface EntryTypeSpec {
  ...
  params: string[];
  tx_param: string;
  lines: { side: string; account: string; amount: string }[];
}
interface Playbook {
  accounts: { path: string; note: string }[];
  entry_types: EntryTypeSpec[];
}

function loadPlaybook(): Playbook {
  return JSON.parse(readFileSync(PLAYBOOK_PATH, "utf8")) as Playbook;
}

// generateSkill renders the SKILL.md text. main.ts writes it to agent/SKILL.md on
// startup, and the Flue brain passes it verbatim as the agent's instructions.
export function generateSkill(): string {
  const pb = loadPlaybook();

  const entryTypeDocs = pb.entry_types
    .map((et) => {
      const lines = et.lines
        .map((l) => `    - ${l.side} \`${l.account}\` = ${l.amount}`)
        .join("\n");
      return [
        `### \`${et.name}\``,
        "",
        et.doc,
        "",
        `- Params: ${et.params.map((p) => `\`${p}\``).join(", ")}`,
        `- Transaction param (idempotency key): \`${et.tx_param}\``,
        `- Lines:`,
        lines,
      ].join("\n");
    })
  ...
  return `# ledger-flow SKILL — the §8 classify/investigate playbook
...
The agent returns \`{entry_type, params}\` as a review recommendation or escalates;
it never affects the ledger. Only a deterministic or explicitly approved path
can send a balanced double-entry to the ledger. The agent never emits raw debits/credits. Its tools are
