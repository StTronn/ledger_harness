# Recovery context and source feeds

> These packages build the recovery and agent view of the world. They are
> **read-only and truth-free**: they exist to supply evidence, not to post to the
> the agent; nothing under it can write to the ledger or reach
> `truth/gl.json` (the import-graph guard enforces it). Pairs with
> `docs/SPEC.md` §8 and `docs/ROADMAP-v2-v3.md` §7.
...
## The agent-facing surface (the `context` CLI)
...
Tier-1 bundles are the registry's output assembled per event/break: the agent's
common cases arrive pre-solved, with citations (`recovered`, `candidates`,
`facts`, each tagged with the producing `policy`). Tier-2 (`entity`) resolves
*anything the agent can name* — an event's raw snapshot + `booked` + edges, an
order with its items, `ratecard/<channel>`, an account path's balance — for the
cases no bundle anticipated. Same binary, same guarantees: read-only, snapshot-
bound, truth-free. The TS brains call these via subprocess (`getEventContext`,
`getBreakContext`, `getEntity`).
