// Package classify is the deterministic PER-EVENT rule engine of ledger-flow
...
//
// # Recovery boundary (SPEC §3, §8)
//
// A recovery candidate uses this same template-shaped Classification data, but
// the judgment agent is not an automatic posting path. The orchestrator binds
// and posts only known rules and candidates validated by the recovery engine.
//
...
// miss carries a human-readable Reason and is handed first to the recovery
// engine. If recovery cannot safely complete it, the judgment agent may review
// it; it is never a panic or a silently-mis-booked entry. The classifier therefore validates
...
//
// This is the shared shape for deterministic rules and recovered candidates.
// Params is keyed to the matching entry type's declared params
...
func Classify(ev ingest.NormalizedEvent) (*Classification, bool, string) {
	...
	case ingest.EventCODDelivery:
		return classifyCODDelivery(ev)
	case ingest.EventCODRemittance:
		return classifyCODRemittance(ev)
	default:
...
//
// A payment with no usable gst_rate is a rule MISS; recovery may fetch and
// validate the rate from the order — not a crash, not a guessed rate.
...
func classifyRefund(ev ingest.NormalizedEvent) (*Classification, bool, string) {
	// PARTIAL refunds are a rule miss REGARDLESS of the rate: the ambiguity is the
	// ENTRY TYPE — one line item returned (refund_reversal) vs a goodwill credit
	// note (price_adjustment) — which no per-event rule can decide. Booking the
	// partial as a refund_reversal would be the wrong-but-balanced failure (the
	// books balance, reconcile stays green, revenue is silently misstated), so the
	// guard fires BEFORE the rate check and hands the judgment to the agent.
	if ev.ParentAmount != nil {
		return nil, false, fmt.Sprintf("%s %s of %s: refund intent (line-item return vs goodwill credit) is not derivable from the event",
			MissPartialRefund, ev.Amount, *ev.ParentAmount)
	}
	rate, ok, reason := gstRateOf(ev)
	if !ok {
		return nil, false, reason
...
// classifyCODDelivery maps a delivered COD shipment to a cod_sale — the COD-rail
// twin of classifyPayment. Revenue is recognised AT DELIVERY (the policy is in
// the playbook doc, never the agent's call), so the gross collected at the door
// is split into net revenue + output GST at the shipment's gst_rate and the gross
// becomes receivable from the courier:
//
//	gross = amount;  (net, gst) = SplitInclusive(gross, gst_rate)
//	params{gross, net, gst, shipment_id};  IK="codsale:"+id;  TxID=shipment_id
//
// A shipment with no usable gst_rate is a rule MISS, exactly as for a payment.
func classifyCODDelivery(ev ingest.NormalizedEvent) (*Classification, bool, string) {
	rate, ok, reason := gstRateOf(ev)
	if !ok {
		return nil, false, reason
	}
	...
	return &Classification{
		...
		Params: map[string]money.Money{
			...
			"net":         net,
			"gst":         gst,
			"shipment_id": txIDPlaceholder,
		},
		IK:     IKFor(ev.Type, ev.ID),
		TxID:   ev.ID,
		Ts:     ev.TS,
		Reason: "cod delivery -> cod_sale",
	}, true, ""
}

// classifyCODRemittance maps a courier remittance to a cod_remittance — the
// COD-rail twin of classifySettlement. The courier nets its collection fee +
// GST-on-fee out of the cash it collected and wires the remainder; the event
// carries net_deposit (Amount), fee (Fee), and gst_on_fee (Tax), and the gross
// the entry clears off the COD receivable is the gross-up of those three:
//
//	net_deposit = amount;  fee = fee;  gst_on_fee = tax
//	gross = net_deposit + fee + gst_on_fee
//	params{net_deposit, fee, gst_on_fee, gross, remittance_id};  IK="codremit:"+id;  TxID=UTR
//
// Crucially this rule clears ONLY the collection-fee portion: any per-shipment
// RTO fee or weight adjustment the courier also netted out is NOT in net_deposit
// + fee + gst_on_fee, so the receivable is left short by exactly those deductions
// — the residual the investigate agent decomposes (ROADMAP §8.3). A remittance
// missing its fee/tax fields is a rule MISS.
func classifyCODRemittance(ev ingest.NormalizedEvent) (*Classification, bool, string) {
	if ev.Fee == nil || ev.Tax == nil {
		return nil, false, "cod remittance missing fee/tax for gross-up"
	}
	netDeposit := ev.Amount
	fee := *ev.Fee
	gstOnFee := *ev.Tax
	...
	return &Classification{
		EntryType: "cod_remittance",
		Params: map[string]money.Money{
			"net_deposit":   netDeposit,
			"fee":           fee,
			"gst_on_fee":    gstOnFee,
			"gross":         gross,
			"remittance_id": txIDPlaceholder,
		},
		IK:     IKFor(ev.Type, ev.ID),
		TxID:   utr,
		Ts:     ev.TS,
		Reason: "cod remittance -> cod_remittance",
	}, true, ""
}

// MissPartialRefund is the stable marker prefix on a partial-refund rule miss.
// Consumers (the closer's skip report, the read model's bundler, the recorded-
// response generator) key the miss CLASS off this prefix — the lightweight form
// of the typed-miss seam until misses grow a structured shape.
const MissPartialRefund = "partial refund:"
